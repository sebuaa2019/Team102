...
int createSubProcess(int read_pipe, int write_pipe) {
    ...
    if (child_pid > 0) {
        ...
        //exit(0);
        execl("/bin/bash", "bash", "/opt/ros/kinetic/bin/rosrun", "team_102", "main", readPortArg.c_str(), writePortArg.c_str(), NULL);
    } else {
...
int main(int argc, char **argv, char **envs) {
    ...
    SCT::Source nodePalpitateTimeout = SCT::Source(SCT::Source::Type::TIMER, 0, 0, qpipe);
    {
        ...
        nodePalpitateSource.setEventHandler([&lastPalpitateTime, pipe_read_node_to_daemon] {
            ...
            if (strcmp(static_cast<const char*>(pdata), "heart") == 0) {
        ...
        nodePalpitateTimeout.setEventHandler([&lastPalpitateTime, &sub_node_pid] {
            printf("hhhhh\n");
    ...
    qchecker.sync([s, &exitSem, envs]{
        ...
        if (child_pid == 0) {
            ...
            dup2(nullfd, 1);
            dup2(nullfd, 2);
            execl("/usr/bin/python", "python", "/opt/ros/kinetic/bin/roslaunch", "team_102", "team102.launch", NULL);
        }
        ts.tv_sec += 10;
